...
import {
  LeaderboardChallengesApiModel,
  LeaderboardProfileChallenges,
} from '../models/LeaderboardProfile';
import { LeaderboardProfileType } from '../models/LeaderboardProfile.type';

export const mapChallengesApiResponse = (data: LeaderboardChallengesApiModel[]): LeaderboardProfileChallenges[] => {
    return data
      .filter(user => user.username && user.name && user.challengesComplete !== undefined) // Filter items based on conditions
      .map(user => ({
        ...
        challengesComplete: user.challengesComplete ?? 0,
